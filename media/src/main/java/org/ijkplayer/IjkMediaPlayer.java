...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    ...
    private static final IjkLibLoader sLocalLibLoader = new IjkLibLoader() {
        @Override
        public void loadLibrary(String libName, boolean isFirstV7) throws UnsatisfiedLinkError, SecurityException {
            if (isFirstV7) {
                NativeLibLoadHelper.simpleLoad(libName, true);
            } else {
                NativeLibLoadHelper.simpleLoad(libName);
            }
        }
    };
    ...
    public static void loadLibrariesOnce(IjkLibLoader libLoader) {
        synchronized (IjkMediaPlayer.class) {
            if (!mIsLibLoaded) {
                ...
                //libLoader.loadLibrary("music3d",false);
                libLoader.loadLibrary("ijkffmpeg",false);
                libLoader.loadLibrary("ijksdl",true);//优先V7
                //libLoader.loadLibrary("openal",false);
                //libLoader.loadLibrary("openalwrapper",false);
                libLoader.loadLibrary("ijkplayer",true);//优先V7
                mIsLibLoaded = true;
            }
        }
    }
    ...
    @Override
    public void setSurface(Surface surface, View screenOnView) {
        this.screenOnView=screenOnView;
        setSurface(surface);
    }
    ...
    public static AudioTrack createAudioTrack(int encoding, int channelMask, int sampleRate, int bufferSizeInBytes, int mode, int sessionId){
        AudioTrack audioTrack = PlayHelper.get().getLogAudioTrack(encoding,channelMask,sampleRate,bufferSizeInBytes,mode,sessionId,audioAttributes);
        bindAudioPlayerInfo(audioTrack);
        return audioTrack;
    }

    private static void bindAudioPlayerInfo(AudioTrack kwAudioTrack) {
       PlayHelper.get().bindLogPlayInfo(kwAudioTrack);
    }

    @CalledByNative
    public void setAudioAttributes(AudioAttributes audioAttributes){
        _setAudioAttributes(true);
    ...
    /**
     * 开启/关闭裁剪功能：默认实际视频宽度的一半，居中显示
     * @param enable
     */
    public native void setShowMiddle(boolean enable);
    public native void _setDisplayDisable(boolean enable);
    public native byte[] getShortCut();
    ...
    /** 只执行停止操作，不执行取消屏幕常亮,秀场异步停止使用 */
    public void onlyStop() throws IllegalStateException {
        _stop();
    }
    ...
    private void updateSurfaceScreenOn() {
        ...
        }else if (screenOnView!=null){
            screenOnView.setKeepScreenOn(mScreenOnWhilePlaying && mStayAwake);
        }
    }
    ...
    public void release() {
        ...
        resetListeners();
        _release();
        this.screenOnView=null;
    }
    /** 只执行回收操作，不执行取消屏幕常亮,秀场异步停止使用 */
    ...
    public MediaInfo getMediaInfo() {
        ...
        mediaInfo.mMediaPlayerName = "org/ijkplayer";
    ...
    //audio 3d process
    /*
    enable or disable audio 3D process
     */
    public native void set3DSound_enable(boolean enable);
    /*
    使用方法：
    //LEFTFRONT = 	0,
    //RIGHTFRONT=  	1,
    ...
    //LEFTBACK  =	3,
    //RIGHTBACK	=	4,
    /////////mask & ( 0x01 << AUDIO_PROCESS_CHANNEL_ID)
     */
    public native int set3DSound_playChannels(int mask);
    public native int set3DSound_pauseChannels(int mask);
    public native int get3DSound_state();
    ...
    private static class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MEDIA_BUFFERING_UPDATE:
                ...
                // Log.e(TAG, "Buffer (" + percent + "%%) + bufferPosition + "/" + duration);
    ...
    public static native void updateEqParam(boolean isEnable, int preAmp, int[] bands);
    public static native void setEffectType(int type);

    public static native void setSpectrum(boolean enable, Spectrum callback);

    public native Bundle _getHiFiParameters();
    public native int _setHiFiParameters(Bundle params);
    /**
     * 设置汽车音效参数
     * @param bands
     * @param nSpeakerCount
     * @param nLFreqSpkCompMode
     * @param nHFreqSpkCompMode
     * @param nBassGain
     * @param nClarity
     * @param n360Surround
     * @param nImageEnhance
     * @return
     */
    public native boolean setCarEffxParam(int[] bands, int nSpeakerCount, int nLFreqSpkCompMode, int nHFreqSpkCompMode, int nBassGain, int nClarity, int n360Surround, int nImageEnhance);

    /**
     * 设置是否适用新的attribute
     * @param enable
     */
    public native void _setAudioAttributes(boolean enable);
}
