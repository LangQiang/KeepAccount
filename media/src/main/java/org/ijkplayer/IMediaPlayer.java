...
public interface IMediaPlayer {
    ...
    int MEDIA_INFO_AUTOSTOPPED = 704;
    ...
    int MEDIA_ERROR_ENOENT	= 2;	//文件不存在(No such file or directory)
    /** File or network related operation errors. */
    int MEDIA_ERROR_EIO = 5;
    int MEDIA_ERROR_SERVER_DIED = 100;
    int MEDIA_ERROR_NETWORK_UNREACHABLE = 101;  //Network unreachable
    int MEDIA_ERROR_CONNECTION_REFUSED = 111;   //Connection refused
    ...
    interface OnErrorListener {
        boolean onError(IMediaPlayer mp, int what, int extra) throws IOException;
    }

    interface OnInfoListener {
        boolean onInfo(IMediaPlayer mp, int what, int extra) throws IOException;
    }
     ...
     *
     * @param surface
     * @param screenOnView 屏幕常亮控制对象
     */
    void setSurface(Surface surface, View screenOnView);
